...
AvlTree *AvlTreeInsertNode_rec(AvlTree *tree, int key, void *data, int typeSizeOfData)
{
    if (tree == NULL)
    {
        return AvlTreeInit(key, data, typeSizeOfData);
    }
    else
    {
        if (key < tree->key)
        {
            tree->left = AvlTreeInsertNode_rec(tree->left, key, data, typeSizeOfData);
            tree->balanceFactor--;
        }
        else if (key > tree->key)
        {
            tree->right = AvlTreeInsertNode_rec(tree->right, key, data, typeSizeOfData);
            tree->balanceFactor++;
        }
        else
        {
    ...
    {
        if (tree->left->balanceFactor == -1 || tree->left->balanceFactor == 0)
        {
...
AvlTree *AvlTreeInsertNode(AvlTree *tree, int key, void *data, int typeSizeOfData)
{
    ...
    if (tree == NULL)
        ...
        newNode = AvlTreeInit(key, data, tree->typeSizeOf);

    AvlTree *temp = tree;
    AvlTree *previous = tree;
...
void CheckAllocation(void *pointer)
{
    if (pointer == NULL)
    {
        printf("Not enought memory to allocate");
        exit(-1);
    }
}
...
asciinode *build_ascii_tree_recursive(AvlTree *t)
{
    ...
    sprintf(node->label, "%d:{%d}", t->key, t->balanceFactor);
